...
// ================================================================================
    ...
    public function UpdateTestResult(){
        $result['AllTests'] = $this->Test_Model->FetchAllTests();
        $this->load->view('UpdateTestResult',$result);
    }
        // SELECT * from student_mst s,test t where s.standard_id = t.standard_id AND s.subject LIKE CONCAT('%',t.subject,'%')     
    ...
    // Column Headers 
    $ColumnName = array('Student ID','Student Name','Roll Number','Marks Obtained');
    $col = 0;
    ...
    foreach ($studDetails as $value) {   
       $object->getActiveSheet()->setCellValueByColumnAndRow(0,$row,$value->stud_id);
       $object->getActiveSheet()->setCellValueByColumnAndRow(1,$row,$value->stud_name);
       $object->getActiveSheet()->setCellValueByColumnAndRow(2,$row,$value->roll_no);
       $row++;
   }
